...
- Copy and paste this (right-click on the clipboard icon in the upper right of the code box) into a new R file in RStudio (in RStudio select the menu `"File"` -> `"New File"` -> `"R Script"` which will open in the upper left pane by default), and next execute it line-by-line (in RStudio position your cursor on the first line of the R code and then right-click on the `"Run"` icon)
...
- Below are non-smooth and smooth nonparametric density estimates for the *numeric* variable `eruptions` (here we use `hist()` and the np package's `npudens()` function, respectively)
